...
import { clearSyncMetadata, getPullCursor, setLastSyncedAtMs, setPullCursor } from "@/features/sync/changeTracker"
import type { InitialSyncStrategy } from "@/features/sync/types"
import { useSyncStore } from "@/features/sync/store"
import { pullChanges } from "@/features/sync/api"

export async function ensureInitialSync(accessToken: string): Promise<boolean> {
  ...
  if (localHasData && !cloudHasData) {
    ...
    await pullApplyAndPersist(accessToken)
    return true
  }
...
export async function resolveInitialSyncStrategy(accessToken: string, strategy: InitialSyncStrategy): Promise<void> {
...
async function pullApplyAndPersist(accessToken: string): Promise<void> {
